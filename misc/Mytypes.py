...
class Manipulable:
    ...
    def to(self: T, device: torch.device) -> T:
        ...
        for field in fields(self):
            ...
            if isinstance(value, Tensor) or isinstance(value, Manipulable):
                replacements[field.name] = value.to(device)
            elif isinstance(value, list) or isinstance(value, tuple):
                replacements[field.name] = [v.to(device) if (isinstance(v, Tensor) or isinstance(v, Manipulable))
                                            else v for v in value]
            elif isinstance(value, dict):
                replacements[field.name] = {key: v.to(device) if (isinstance(v, Tensor) or isinstance(v, Manipulable))
                                            else v for key, v in value.items()}
