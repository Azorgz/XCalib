...
from ThirdParty.ImagesCameras import Camera, ImageTensor
from . import Cameras, CamerasCfg
from ..frame_sampler import FrameSampler


@dataclass
class InfraParisCfg(CamerasCfg):
    name: Literal["InfraParis"]
    image_shape: tuple[int, int] | None
    root_cameras: list[Path | str]
    cameras_name: list[str] | None
    cameras_id: list[str] | None
    dataset_name: str = "InfraParis"


class InfraParis(Cameras):
    ...
    def read_image(
        ...
        if camera.modality == 'Visible':
            ...
            image = ImageTensor(image[:, :, :int(image.shape[-2]/2)])
        else:
