...
from posenet.model.projection import sample_image_grid
from .common import split_videos, Flows
...
class FlowPredictor(nn.Module, ABC, Generic[T]):
    ...
    def compute_bidirectional_flow(
        ...
        forward_mask = self.compute_consistency_mask(batch, forward)
        forward = self.rescale_flow(forward, flow_shape)
        forward_mask = self.rescale_mask(forward_mask, flow_shape)

        backward_videos = batch.flip(dims=(1,))

        backward = self.forward(backward_videos, backward=True)
