...
from ..intrinsics.common import focal_lengths_to_scaled_intrinsics
from ..projection import projection_frame_to_frame, compute_forward_flow, compute_backward_flow, sample_image_grid
...
class SpatialTransformerOutput:
    ...
    flows: Float[Tensor, "b f-1 c h w"] | None
...
class SpatialTransformer(nn.Module):
    ...
    def forward(
            ...
            global_step: int,
            extrinsics: Float[Tensor, "batch f 4 4"],
            surfaces: Float[Tensor, "batch f p xyz"],
            intrinsics: Float[Tensor, "batch f 3 3"]
        ...
        if extrinsics is not None:
            res['flows'] = self.projection_sequence(batch, extrinsics, surfaces, intrinsics)
        else:
            res['flows'] = None
    ...
    def projection_sequence(self,
                            batch,
                            extrinsics,
                            surfaces,
                            intrinsics):
        h, w = batch.videos.shape[-2:]
        images = batch.videos[:, :-1]
        b, f = images.shape[:2]
        fw_flows = compute_forward_flow(surfaces, extrinsics, intrinsics)
        fw_flows = fw_flows / torch.tensor([w, h], device=fw_flows.device)
        image_flow = grid_sample(rearrange(images, 'b f c h w -> (b f) c h w'),
                            rearrange(fw_flows * 2 - 1, 'b f h w xy -> (b f) h w xy'))
        return rearrange(image_flow, '(b f) c h w -> b f c h w', b=b, f=f)
