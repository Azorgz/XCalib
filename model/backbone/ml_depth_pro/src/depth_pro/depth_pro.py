...
DEFAULT_MONODEPTH_CONFIG_DICT = DepthProConfig(
    ...
    checkpoint_uri="./checkpoint/depth_pro.pt",
...
class DepthPro(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[torch.Tensor]]:
        ...
        features = features.split(8, 1)
        features = torch.cat([f.mean(dim=1, keepdim=True) for f in features], dim=1)
        fov_deg = None
        if hasattr(self, "fov"):
            fov_deg = self.fov.forward(x, features_0.detach())
        return canonical_inverse_depth, fov_deg, features
    ...
    def infer(
        ...
        canonical_inverse_depth, fov_deg, features = self.forward(x)
        if f_px is None:
