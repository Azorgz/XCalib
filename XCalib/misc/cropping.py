...
from posenet.flow import Flows
...
# @dataclass
# class CroppingForFlowCfg(CroppingCfg):
#     flow_scale_multiplier: int


@dataclass
class CroppingForModelCfg(CroppingCfg):
    image_shape: tuple[int, int] | int
    patch_size: int
...
def resize_flow(flows: Flows, shape: tuple[int, int]) -> Flows:
    b, f, h, w, xy = flows.backward.shape
    h, w = shape

    backward = rearrange(flows.backward, "b f h w xy -> (b f) xy h w")
    backward = F.interpolate(backward, (h, w), mode="bilinear", align_corners=False)
    backward = rearrange(backward, "(b f) xy h w -> b f h w xy", b=b, f=f)

    forward = rearrange(flows.forward, "b f h w xy -> (b f) xy h w")
    forward = F.interpolate(forward, (h, w), mode="bilinear", align_corners=False)
    forward = rearrange(forward, "(b f) xy h w -> b f h w xy", b=b, f=f)

    backward_mask = F.interpolate(flows.backward_mask, (h, w), mode="bilinear", align_corners=False)
    forward_mask = F.interpolate(flows.forward_mask, (h, w), mode="bilinear", align_corners=False)

    return replace(flows, backward=backward, forward=forward, forward_mask=forward_mask, backward_mask=backward_mask)
...
def crop_and_resize_batch_for_model(
    ...
    return patch_crop_batch(batch, cfg.patch_size)


# def crop_and_resize_batch_for_flow(batch: Batch, cfg: CroppingForFlowCfg) -> Batch:
