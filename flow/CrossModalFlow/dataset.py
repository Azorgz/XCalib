...
def get_tfs_lighter(args):
    ...
    if args['Weather']:
        tfs += [A.OneOf([A.augmentations.transforms.RandomFog(), A.augmentations.transforms.RandomRain(),
                         A.augmentations.transforms.RandomSnow()]), ]

    if args['ColorJitter']:
        tfs += [A.augmentations.transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.2,
                                                       always_apply=True),
                ]
    ...
    if args['Blur']:
        blur_list = [A.augmentations.transforms.GaussianBlur(blur_limit=(1, 5)),
                     A.augmentations.transforms.MotionBlur(blur_limit=5),
                     A.augmentations.transforms.MedianBlur(blur_limit=5)]
        if args['GlassBlur']:
    ...
    if args['Occlusion']:
        tfs_occ += [
            A.OneOf(
                [A.augmentations.transforms.RandomShadow(), A.augmentations.transforms.RandomSunFlare(src_radius=100)],
                p=0.8),
            A.OneOf([A.augmentations.transforms.GridDropout(random_offset=True, ratio=0.2),
                     A.augmentations.transforms.CoarseDropout(), A.augmentations.transforms.Cutout()])
        ]
...
class BaseDataset(Dataset):
    def __init__(self, training=True, patch_h=256, patch_w=256,
                 augment_args=['random_pos', 'random_flip', 'random_rotate'], tfs_args=None, cross_modal_aug_rate=1.0):
...
class YoutubeVOSDataset(BaseDataset):
    def __init__(self,
                 root_path='/home/godeta/Images/youtube_vos', f_range=10,
                 training=True, patch_h=256, patch_w=256, augment_args=['random_pos', 'random_flip', 'random_rotate'],
                 tfs_args=None, cross_modal_aug_rate=1.0):
        super(YoutubeVOSDataset, self).__init__(training, patch_h, patch_w, augment_args, tfs_args,
                                                cross_modal_aug_rate)
        assert (training)
        ...
        for d in os.listdir(os.path.join(root_path, 'train_all_frames', 'JPEGImages')):
            ...
            min_n = min(numbers)
            try:
                assert ((max_n - min_n) % (len(numbers) - 1) == 0)
                l.append((os.path.join(dir_path, '%05d.jpg'), min_n, max_n, (max_n - min_n) / (len(numbers) - 1)))
            except AssertionError:
                print(f'Warning: Mismatch in number of frames in {d}. Skipping...')
                continue
    ...
    def __getitem__(self, idx):
        path, cnt_min, cnt_max, step = self.l[idx]
        while True:
            xx = random.randint(cnt_min / step, cnt_max / step) * step
            yy = random.randint(-self.f_range / step, self.f_range / step) * step
            yy = xx + yy
